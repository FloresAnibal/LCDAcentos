...
int LCDAcentos::_buscarCaracterEspecial(byte byte1, byte byte2) {
  for (int i = 0; i < MAX_CARACTERES_ESPECIALES; i++) {
...
bool LCDAcentos::_asignarSlotACaracter(int indiceCaracter) {
  ...
  for (int slot = 0; slot < TOTAL_SLOTS_LCD; slot++) {
    if (!_slotReservado[slot]) {
      ...
      if (!slotEnUso) {
        _slotsUsados[indiceCaracter] = slot;
        return true;
      }
    }
  }
    ...
    if (i + 1 < texto.length()) {
      ...
      if (indiceCaracter >= 0 && _slotsUsados[indiceCaracter] == 255) {
        if (_asignarSlotACaracter(indiceCaracter)) {
          ...
          _lcd->createChar(_slotsUsados[indiceCaracter], 
                          CARACTERES_ESPECIALES[indiceCaracter].disenio);
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < texto.length(); i++) {
    ...
    if (i + 1 < texto.length() && _esCaracterUTF8(primerByte)) {
      ...
      if (indiceCaracter >= 0 && _slotsUsados[indiceCaracter] != 255) {
        ...
        _actualizarCursor();
        _lcd->write(_slotsUsados[indiceCaracter]);
...
bool LCDAcentos::_esCaracterUTF8(byte b) {
